...
const baseSensors = {mapKpa:178, railPsi:110.2, baroKpa:101.3};
let mockCfg = {
  injHz: 50,
  dutyClamp: 60,
  dpMinPsi: 50,
...
function readCurveAtWith(curve, kpa){
  ...
  if(kpa <= curve[0].kpa) return curve[0].pct;
...
function computeMockStatus(){
  ...
  const baroKpa = (mockCfg.forceBaroKpa >= 0) ? mockCfg.forceBaroKpa : baseSensors.baroKpa;
  ...
  const sprayActive = dutyCmd > 0.5;
  const dpFault = !!(mockCfg.forceDpFault || (sprayActive && (dpPsi < (Number(mockCfg.dpMinPsi)||0))));
  const timingCut = !!(mockCfg.forceTimingCut || dpFault || (levelLow && sprayActive));
  const dpBoostHold = !!(mockCfg.forceDpBoostHold || dpFault);
  const boostOn = !(levelLow || timingCut || mockCfg.forceBoostCut || dpBoostHold);

  let pumpOn = false;
  if(mockCfg.forcePump === 1) pumpOn = false;
  else if(mockCfg.forcePump === 2) pumpOn = !timingCut && !mockCfg.forceBoostCut;
  else pumpOn = sprayActive && !timingCut && !mockCfg.forceBoostCut;

  const injOn = sprayActive && !timingCut && !mockCfg.forceBoostCut;
  ...
  else if(!boostOn && levelLow) safetyReason = 'Low Level';
  ...
  return {
    ...
    pumpPrimeMs: mockCfg.primeMs,
    pumpPrimed: sprayActive,
    levelRaw: levelLow,
    dpMinPsi: mockCfg.dpMinPsi,
    dpArmPct: mockCfg.dpArmPct,
    dpRecover: mockCfg.dpRecover,
    lvlDebounce: mockCfg.lvlDebounce,
    primeMs: mockCfg.primeMs,
    injLb: mockCfg.injLb,
    mixMethPct: mockCfg.mixMethPct,
    targetInjectorDp: mockCfg.targetInjectorDp,
    safetyState,
    safetyReason,
    faultReason: safetyReason || 'None',
...
function applyStatus(s){
  ...
  const faultText = sr || fr || (status.faultActive ? "FAULT" : "OK");
